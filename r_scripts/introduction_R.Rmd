...
```{r, eval = F}
# t-test
t.test(ACT_MATH~GENDER, data = pretest)

# As a regression
m0 <- lm(ACT_MATH ~ GENDER, data = pretest)
summary(m0)

pretest$white <- ifelse(pretest$ETHDESCR == "White     ", "White","Non-White")
table(pretest$white)
pretest$red.ethnic <- ifelse(pretest$ETHDESCR == "White     ", "White",
                            ifelse(pretest$ETHDESCR ==  "Black     ", "Black", "Other"))

red.ethnic <- ifelse(pretest$ETHDESCR == "White     ", "White",
                     ifelse(pretest$ETHDESCR ==  "Black     ", "Black", "Other"))
...
t.test(ACT_MATH~GENDER, data = pretest, var.equal = FALSE)
...
m1 <- lm(CUM_GPA ~ ACT_TOTL, data = pretest)
summary(m1)
...
# Multiple regression
m2 <- lm(CUM_GPA ~ HSPR + STATGRAD, data = pretest)
summary(m2)

# ANOVA
m3 <- lm(CUM_GPA ~ GENDER + ETHDESCR + GENDER*ETHDESCR, data = pretest)
anova(m3)

# Fit the model with everything in it except STATGRAD and id
m4 <- lm(CUM_GPA ~ . - id - STATGRAD, data = pretest)
summary(m4)

# Do stepwise selection, but can't have missing data
# Omit missing data
pretest.m <- na.omit(pretest)
m5 <- lm(CUM_GPA ~ . - id - STATGRAD, data = pretest.m)
